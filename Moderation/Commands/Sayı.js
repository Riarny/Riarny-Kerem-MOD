module.exports = {
 ...
 run: async ({client, msg, args, author, MessageEmbed}) => {
   
  let kelime = args[0]
  if(!kelime) return client.timemessage(client.normalEmbed(`Bir kelime belirt ve tekrardan dene!`, msg), msg.channel.id, 5000)
  const filter = (reaction, user) => {return ["👌"].includes(reaction.emoji.name) && user.id === msg.author.id;};
  if(msg.guild.members.cache.filter(r=>r.user.tag.includes(kelime)).size === 0) return client.timemessage(client.normalEmbed(`Kullanıcı isminde \`${kelime}\` geçen kullanıcı yok.`, msg), msg.channel.id, 5000)
  msg.channel.send(`Adında \`${kelime}\` geçen toplamda ${msg.guild.members.cache.filter(r=>r.user.tag.includes(kelime)).size} kişi var. Tüm üyeleri görmek istiyorsanız 👌 emojisine basınız.\n\n30 saniye içinde iptal edilecektir.`).then(m => m.react("👌").then(s =>m.awaitReactions(filter, { max: 1, time: 30000, errors: ["time"] }).then(collected => {const reaction = collected.first()
