module.exports = {
 ...
 run: async ({client, msg, args, cfg, author, MessageEmbed}) => {
   ...
   let keys = Object.keys(uye.presence.clientStatus).join(',')
   let cihazisim = keys.replace(`mobile`,`Mobil Telefon`).replace(`desktop`,`Bilgisayar (Uygulama)`).replace(`web`,`İnternet Tarayıcısı`)
