module.exports = {
 ...
 run: async ({client,SetupDatabase, msg, cfg, args,  author, MessageEmbed, guild, Puan, moment, CoinDatabase, CezapuanDatabase, YetkiliKayıtDatabase, CezaSayıDatabase, ControlsDatabase, RegisterDatabase, StaffDatabase, görevDatabase}) => {
   ...
   if (ladyRoles.length === 0) return client.message(client.normalEmbed(`Henüz kız rolünün kurulumu yapılmamış, kurulumu yap ve tekrardan dene!`, msg), msg.channel.id)
   if (vegas.roles.cache.some(r => ladyRoles.includes(r.id))) return client.timemessage(client.normalEmbed(`Kayıt etmek istediğin ${vegas} üyesi zaten daha önce kayıt edilmiş. Yanlış teyit işlemi uygulandıysa üst yönetime ulaşmaktan çekinme!`, msg), msg.channel.id, 10000)
  ...
  if(!isim) return client.timemessage(client.normalEmbed(`Üyeye geçerli bir isim gir ve tekrardan dene!`, msg), msg.channel.id, 5000)
   ...
   if (registerLimit >= registerLimitNumber) {
    client.react(msg, "red")
    client.message(client.normalEmbed(`1 dakika içerisinde çok fazla kayıt işlemi uyguladığınız için komut geçici olarak kullanımınıza kapatılmıştır.`, msg), msg.channel.id)   
   }
   ...
   await vegas.roles.set(ladyRoles)
   await vegas.setNickname(`• ${args[1].charAt(0).replace('i', "İ").toUpperCase() + args[1].slice(1).toLowerCase()}`).catch(() => { })
   i//if (vegas.user.username.toLowerCase().includes("gng") || vegas.user.discriminator === "0232" || vegas.user.discriminator === "2320") vegas.roles.add("979121908207485009");
   if (taggedRole !== "") if (!cfg.Tag.Tags.some(x => vegas.user.tag.includes(x))) vegas.roles.add(taggedRole);
   const Vegas = new MessageEmbed().setColor("RANDOM").setAuthor(msg.author.tag, msg.member.user.avatarURL({dynamic: true})).setFooter(`Üyenin ceza puanı: ${totalCezapuan}`).setDescription(`${vegas} adlı kullanıcı başarıyla kız olarak kaydedildi.`)
   msg.channel.send(Vegas)
   if (taggedRole !== "") if (vegas.user.username.includes(guildTag)) vegas.roles.add(taggedRole) 
   ...
   if(!RegisterData) {let newRegisterDatabase = new RegisterDatabase({guildID: msg.guild.id, userID: vegas.id, authorID: msg.author.id, isimler: [{ name: `${vegas.displayName}`, type: `<@&${ladyRoles[0]}>`, staff: msg.author.id }]}).save();} else{
   RegisterData.isimler.push({ name: `${vegas.displayName}`, type: `${ladyRoles[0]}`, staff: msg.author.id }); 
   RegisterData.save();}     
