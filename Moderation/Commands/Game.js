...
module.exports = {
 ...
 run: async ({client, msg, args, author, GeneralDatabase, SetupDatabase}) => {
  ...
  let taggedTrue = res && res.taggedRole ? res.taggedRole : "" 
  if (!msg.member.roles.cache.some(r => commandsTrue.includes(r.id)) && !msg.member.roles.cache.has(taggedTrue) && !msg.member.hasPermission(8)) return client.timemessage(client.normalEmbed(`Bu özellik sadece taglı üyelerimiz içindir. Tag alıp kullanabilirsin.`, msg), msg.channel.id, 5000) 
  ...
  if (args[0] == 'yt' || args[0] == 'youtube' || args[0] == 'Youtube') {
   fetch(`https://discord.com/api/v8/channels/${msg.member.voice.channel.id}/invites`, { method: "POST", body: JSON.stringify({ max_age: 86400, max_uses: 0, target_application_id: "755600276941176913", target_type: 2, temporary: false, validate: null }), headers: { "Authorization": `Bot ${client.token}`, "Content-Type": "application/json" } }).then(res => res.json()).then(invite => {
   ...
   fetch(`https://discord.com/api/v8/channels/${msg.member.voice.channel.id}/invites`, { method: "POST", body: JSON.stringify({ max_age: 86400, max_uses: 0, target_application_id: "773336526917861400", target_type: 2, temporary: false, validate: null }), headers: { "Authorization": `Bot ${client.token}`, "Content-Type": "application/json" } }).then(res => res.json()).then(invite => {
   ...
   fetch(`https://discord.com/api/v8/channels/${msg.member.voice.channel.id}/invites`, { method: "POST", body: JSON.stringify({ max_age: 86400, max_uses: 0, target_application_id: "755827207812677713", target_type: 2, temporary: false, validate: null }), headers: { "Authorization": `Bot ${client.token}`, "Content-Type": "application/json" }}).then(res => res.json()).then(invite => {
  ...
  fetch(`https://discord.com/api/v8/channels/${msg.member.voice.channel.id}/invites`, { method: "POST", body: JSON.stringify({ max_age: 86400, max_uses: 0, target_application_id: "814288819477020702", target_type: 2, temporary: false, validate: null }), headers: { "Authorization": `Bot ${client.token}`, "Content-Type": "application/json" } }).then(res => res.json()).then(invite => {
