module.exports = {
 ...
 run: async ({client, msg, args, author, MessageEmbed, uye, moment, MsgDeleteDatabase, SetupDatabase}) => {
  ...
  let list = MsgDeleteData && MsgDeleteData.msg ? MsgDeleteData.msg.map((value, index) => `\`⦁\` Mesaj Sahibi: <@!${value.userID}>\n\`⦁\` Mesaj İçeriği: ${value.msg.length >= 100 ? "Mesaj uzun olduğu için gösteremiyorum." : value.msg.replace("`", "")}\n\`⦁\` Mesaj Silinme Tarihi: ${moment(value.date).locale("TR").format("LLL")} (${moment(value.date-10800000).locale("TR").fromNow()})`).reverse().slice(0, 5).join("\n─────────────────\n") : ""
  client.message(client.normalEmbed(`${msg.channel} kanalında silinen son **5 mesaj:**\n\n ${list}`, msg), msg.channel.id)
  return}
  ...
  if(!(MsgDeleteData && MsgDeleteData.msg)) return client.timemessage(client.normalEmbed(`Veritabanında bu kanala ait silinmiş bir mesaj bulamıyorum.`, msg), msg.channel.id, 5000)
  let list = MsgDeleteData && MsgDeleteData.msg ? MsgDeleteData.msg.map((value, index) => `\`⦁\` Mesaj Sahibi: <@!${value.userID}>\n\`⦁\` Mesaj İçeriği: ${value.msg.length >= 100 ? "Mesaj uzun olduğu için gösteremiyorum." : value.msg.replace("`", "")}\n\`⦁\` Mesaj Silinme Tarihi: ${moment(value.date).locale("TR").format("LLL")} (${moment(value.date-10800000).locale("TR").fromNow()})`).reverse().slice(0, 5).join("\n─────────────────\n") : ""
  client.message(client.normalEmbed(`${client.channels.cache.get(Channel.id)} kanalında silinen son **5 mesaj:**\n\n ${list}`, msg), msg.channel.id)}}
