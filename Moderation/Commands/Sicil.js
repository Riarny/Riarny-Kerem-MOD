...
module.exports = {
 ...
 run: async ({client, msg, args, author, MessageEmbed, uye, fs, CezaDatabase, CezaSayıDatabase, CezapuanDatabase, PunitiveStatus, moment, SetupDatabase}) => {
  ...
  try{if (!msg.member.roles.cache.some(r => commandsTrue.includes(r.id)) && !msg.member.roles.cache.some(r => botCommandsTrue.includes(r.id)) &&!msg.member.hasPermission(8)) return client.timemessage(client.yetersizYetki(msg, msg), msg.channel.id, 5000)                              
  ...
  await CezaDatabase.find({ userID: uye.id }).sort({ cezaID: "descending" }).exec(async (err, res) => {
   ...
   let dataxe = [ ["ID", "Ceza", "Tarih", "Bitiş", "Yetkili", "Sebep"] ];
   let config = { border: { topBody: ``, topJoin: ``, topLeft: ``, topRight: ``, bottomBody: ``, bottomJoin: ``, bottomLeft: ``, bottomRight: ``, bodyLeft: `│`, bodyRight: `│`, bodyJoin: `│`, joinBody: ``, joinLeft: ``, joinRight: ``, joinJoin: `` } };
    res.map(x => { datax.push([x.cezaID, moment(x.date).locale("TR").format("LLL"), x.Type, x.Reason]) })
    res.map(x => { dataxe.push([x.cezaID, x.Type, moment(x.date).locale("TR").format("LLL"), moment(x.finishDate).locale("TR").format("LLL"), client.users.cache.get(x.authorID).tag, x.Reason]) })
