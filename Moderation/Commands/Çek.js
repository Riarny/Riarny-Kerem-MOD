module.exports = {
 ...
 run: async ({client, msg, cfg, args, author, guild, MessageEmbed, uye, SetupDatabase}) => {
  ...
  if (msg.member.roles.cache.some(r => commandsTrue.includes(r.id)) && msg.member.hasPermission(268435456) && msg.member.hasPermission(8)) {client.çek(uye, msg.member); client.timemessage(client.normalEmbed(`Başarıyla ${uye} adlı üye kanalınıza taşındı.`, msg), msg.channel.id, 5000); client.react(msg, "tick"); return}
  ...
  if (reaction.emoji.id === "976939143290572901") {client.react(msg, "tick")
  ...
  if (reaction.emoji.id === "976939024524673064") {client.react(msg, "red"); client.timemessage(client.normalEmbed(`${msg.author}, Belirttiğin kişi bulunduğun ses kanalına gelmek istemiyor bu sebepten ötürü bulunduğun ses kanalına taşıyamadım. `, msg), msg.channel.id, 5000)
